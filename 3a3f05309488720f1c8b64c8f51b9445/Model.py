...
def compute_Model_params(out_label='', mode='default'):
    ...
    for cb in binzegger_E_pops:
        ...
        if "E"+cell_layer not in transform_cell_type:
            transform_cell_type.update({("E"+cell_layer):{cb}})
        else:
            transform_cell_type["E"+cell_layer].update({cb})
    for cb in binzegger_b_pops:
        cell_layer = re.sub("\D", "", re.sub("\(.*\)", "", cb))
        if "P"+cell_layer not in transform_cell_type:
            transform_cell_type.update({("P"+cell_layer):{cb}})
        else:
            transform_cell_type["P"+cell_layer].update({cb})
    for cb in binzegger_nb_pops:
        cell_layer = re.sub("\D", "", re.sub("\(.*\)", "", cb))
        if "H"+cell_layer not in transform_cell_type:
            transform_cell_type.update({("H"+cell_layer):{cb}})
        else:
            transform_cell_type[("H"+cell_layer)].update({cb})     
    for cb in binzegger_nb_pops:
        cell_layer = re.sub("\D", "", re.sub("\(.*\)", "", cb))
        if "V"+cell_layer not in transform_cell_type:
            transform_cell_type.update({("V"+cell_layer):{cb}})
        else:
            transform_cell_type[("V"+cell_layer)].update({cb})     
    ...
    for target_area, target_pop, source_area, source_pop in product(area_list, population_list,
        ...
        if 'E' in source_pop:
            synapse_weights_mean[target_area][target_pop][source_area][
                source_pop] = PSC_e_over_PSP_e * g_dict[source_pop] * PSP_e
        ...
        if 'H' in source_pop:
            synapse_weights_mean[target_area][target_pop][source_area][
                source_pop] = PSC_h_over_PSP_h * g_H * g_dict[source_pop] * PSP_e
            # print("weight=",synapse_weights_mean[target_area][target_pop][source_area][source_pop])
            # print("PSC_h_over_PSP_h=",PSC_h_over_PSP_h)
        if 'P' in source_pop:
            if 'P' in target_pop:
                synapse_weights_mean[target_area][target_pop][source_area][
                    source_pop] = PSC_p_over_PSP_p * g_P * g_dict[source_pop] * PSP_e 
                # print("PSC_p_over_PSP_p=",PSC_p_over_PSP_p)
            else:
                synapse_weights_mean[target_area][target_pop][source_area][
                    source_pop] = PSC_p_over_PSP_p * g_P * g_dict[source_pop] * PSP_e
        ...
        if "S" in source_pop:
            synapse_weights_mean[target_area][target_pop][source_area][
                source_pop] = PSC_s_over_PSP_s * g_S * g_dict[source_pop] * PSP_e
            
        if "V" in source_pop:
            synapse_weights_mean[target_area][target_pop][source_area][
                source_pop] = PSC_v_over_PSP_v * g_V * g_dict[source_pop] * PSP_e
            # print("PSC_s_over_PSP_s=",PSC_s_over_PSP_s)
        ...
        if  target_pop == 'H1':
            if source_pop[0] == 'E':
                synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 3.9*synapse_weights_mean[target_area][target_pop][source_area][source_pop]        
            else:
        ...
        if source_pop[0] == 'E' and target_pop == 'E23':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.8*synapse_weights_mean[target_area][target_pop][source_area][source_pop]        
        if source_pop[0] == 'E' and target_pop == 'P23':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.48*synapse_weights_mean[target_area][target_pop][source_area][source_pop]
        if source_pop[0] == 'E' and target_pop == 'S23':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 1.*synapse_weights_mean[target_area][target_pop][source_area][source_pop]
        ...
        if source_pop[0] == 'E' and target_pop == 'E4':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 1.66*synapse_weights_mean[target_area][target_pop][source_area][source_pop]        
        if source_pop[0] == 'E' and target_pop == 'S4':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.24*synapse_weights_mean[target_area][target_pop][source_area][source_pop]
        if source_pop[0] == 'E' and target_pop == 'V4':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.44*synapse_weights_mean[target_area][target_pop][source_area][source_pop]
        if source_pop[0] == 'E' and target_pop == 'P4':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.78*synapse_weights_mean[target_area][target_pop][source_area][source_pop]            
        
        #调整5层的输入
        if source_pop[0] == 'E' and target_pop == 'E5':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.91*synapse_weights_mean[target_area][target_pop][source_area][source_pop]
        if source_pop[0] == 'E' and target_pop == 'S5':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.48*synapse_weights_mean[target_area][target_pop][source_area][source_pop]
        if source_pop[0] == 'E' and target_pop == 'V5':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 1.2*synapse_weights_mean[target_area][target_pop][source_area][source_pop]     
        if source_pop[0] == 'E' and target_pop == 'P5':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 1.05*synapse_weights_mean[target_area][target_pop][source_area][source_pop]       
        ...
        if source_pop[0] == 'E' and target_pop == 'E6':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 1.12*synapse_weights_mean[target_area][target_pop][source_area][source_pop]        
        if source_pop[0] == 'E' and target_pop == 'S6':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.63*synapse_weights_mean[target_area][target_pop][source_area][source_pop]
        if source_pop[0] == 'E' and target_pop == 'V6':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.5*synapse_weights_mean[target_area][target_pop][source_area][source_pop]
        if source_pop[0] == 'E' and target_pop == 'P6':
            synapse_weights_mean[target_area][target_pop][source_area][source_pop] = 0.42*synapse_weights_mean[target_area][target_pop][source_area][source_pop]        
        #调整
