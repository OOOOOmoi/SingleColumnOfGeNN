...
# 计算放电率的时间窗口（ms）
bin_size = 50  # 5ms 时间窗口
smooth_window = 500 # 20ms 滑动时间窗口
time_threshold = 500  # 忽略前 50ms 的数据

def read_spike_data(file_name, time_threshold):
...
for idx, pop in enumerate(PopList):
    ...
    # 读取前 5% 神经元的数据
    times, neuron_ids = read_spike_data(file_path, time_threshold)
    ...
    time_range = np.arange(time_threshold, max_time + 1, bin_size)  # 以 bin_size 为步长
    firing_rate = np.zeros_like(time_range, dtype=float)

    for i, t in enumerate(time_range):
        # 统计 `t - smooth_window` 到 `t` 时间段内的 spike 数量
        firing_rate[i] = np.sum((times >= (t - smooth_window)) & (times < t)) / (smooth_window / 10000.0) / max(original_neuron_ids)

    # 🎯 绘制放电率变化（折线图）
    axes[1].plot(time_range, firing_rate, color=color, label=group_name, linewidth=2)
